...
static int uhdlc_init(struct ucc_hdlc_private *priv)
{
	...
	/* Loopback mode */
	if (priv->loopback) {
		gumr = in_be32(&priv->uf_regs->upsmr);
		...
		struct ucc_hdlc_private *priv, struct ucc_hdlc_info *uh_info)
{
	...
	if (!np2) {
		dev_err(priv->dev, "No si property\n");
		return -EINVAL;
	}
	of_address_to_resource(np2, 0, &res);
	priv->si_regs = ioremap(res.start, res.end - res.start + 1);
	of_node_put(np2);
	...
	if (!np2) {
		ret = -EINVAL;
		dev_err(priv->dev, "No siramproperty\n");
		goto err_miss_siram_property;
	}
	of_address_to_resource(np2, 0 , &res);
	priv->siram = ioremap(res.start, res.end - res.start + 1);
	of_node_put(np2);

	if (siram_init_flag == 0) {
		memset(priv->siram, 0,  res.end - res.start + 1);
		siram_init_flag = 1;
	}
