/*
 * A hwmon driver for the Accton as5700 96x fan contrl
 *
...
static struct accton_as5700_96x_fan  *fan_data = NULL;

struct accton_as5700_96x_fan {
    struct device   *dev;
...
static void accton_as5700_96x_fan_update_device(struct device *dev);
static int accton_as5700_96x_fan_read_value(u8 reg);
static int accton_as5700_96x_fan_write_value(u8 reg, u8 value);
...
static struct attribute *accton_as5700_96x_fan_attributes[] = {
             ...
             char *buf)
{
    ...
    accton_as5700_96x_fan_update_device(dev);
...
static ssize_t fan_set_duty_cycle(struct device *dev, struct device_attribute *da,
    ...
    accton_as5700_96x_fan_write_value(fan_reg[idx], fan_duty_cycle_to_reg_val(value));
    
    return count;
}

static const struct attribute_group accton_as5700_96x_fan_group = {
    .attrs = accton_as5700_96x_fan_attributes,
};
...
static void accton_as5700_96x_fan_update_device(struct device *dev)
{
    ...
    if (time_after(jiffies, fan_data->last_updated + HZ + HZ / 2) || 
        ...
        dev_dbg(fan_data->dev, "Starting accton_as5700_96x_fan update\n");

        /* Update fan data
         */
        for (i = 0; i < ARRAY_SIZE(fan_data->reg_val); i++) {
            int status = accton_as5700_96x_fan_read_value(fan_reg[i]);
...
{	
    ...
    accton_as5700_96x_fan_update_device(fan_data->dev);
...
static void set_fan_speed(int duty_cycle) 
{
    ...
    accton_as5700_96x_fan_write_value(fan_reg[1], fan_duty_cycle_to_reg_val(duty_cycle));
}
...
static int fan_speed_ctrl_init(void)
{
	...
	fanctrl_tsk = kthread_create(fan_speed_ctrl_routine, NULL, "accton_as5700_96x_fanctl");
...
static int __init accton_as5700_96x_fan_init(void)
{
    int status = 0;

    fan_data = kzalloc(sizeof(struct accton_as5700_96x_fan), GFP_KERNEL);
    if (!fan_data) {
        status = -ENOMEM;
        goto exit;
    }
    ...
    status = sysfs_create_group(&fan_data->dev->kobj, &accton_as5700_96x_fan_group);
    if (status) {
        goto exit_unregister;
    }

    dev_info(fan_data->dev, "accton_as5700_96x_fan\n");
...
static void __exit accton_as5700_96x_fan_exit(void)
{
    fan_speed_ctrl_cleanup();
	
    sysfs_remove_group(&fan_data->dev->kobj, &accton_as5700_96x_fan_group);
	hwmon_device_unregister(fan_data->dev);
    kfree(fan_data);
}

MODULE_AUTHOR("Brandon Chuang <brandon_chuang@accton.com.tw>");
MODULE_DESCRIPTION("accton_as5700_96x_fan driver");
MODULE_LICENSE("GPL");

module_init(accton_as5700_96x_fan_init);
module_exit(accton_as5700_96x_fan_exit);
