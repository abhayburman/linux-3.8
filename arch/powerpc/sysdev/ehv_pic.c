...
void ehv_pic_unmask_irq(unsigned int irq)
{
...
void ehv_pic_mask_irq(unsigned int irq)
{
	unsigned int src = virq_to_hw(irq);
...
void ehv_pic_end_irq(unsigned int irq)
{
	unsigned int src = virq_to_hw(irq);
...
void ehv_pic_direct_end_irq(unsigned int irq)
{
	out_be32(mpic_percpu_base_vaddr + MPIC_EOI / 4, 0);
}

int ehv_pic_set_affinity(unsigned int irq, const struct cpumask *cpumask)
{
	unsigned int src = virq_to_hw(irq);
...
int ehv_pic_set_irq_type(unsigned int virq, unsigned int flow_type)
{
...
static struct irq_chip ehv_pic_irq_chip = {
	.mask		= ehv_pic_mask_irq,
	.unmask		= ehv_pic_unmask_irq,
	.eoi		= ehv_pic_end_irq,
	.set_type	= ehv_pic_set_irq_type,
};

static struct irq_chip ehv_pic_direct_eoi_irq_chip = {
	.mask		= ehv_pic_mask_irq,
	.unmask		= ehv_pic_unmask_irq,
	.eoi		= ehv_pic_direct_end_irq,
	.set_type	= ehv_pic_set_irq_type,
};
			 ...
			 irq_hw_number_t hw)
{
	...
	set_irq_chip_data(virq, ehv_pic);
	/*
...
void __init ehv_pic_init(struct device_node *node, int coreint_flag)
{
	...
	ehv_pic->irqhost = irq_alloc_host(of_node_get(node),
				IRQ_HOST_MAP_LINEAR,
				NR_EHV_PIC_INTS,
				&ehv_pic_host_ops, 0);

	if (ehv_pic->irqhost == NULL) {
		of_node_put(node);
		return;
	}
	...
	if (np) {
		...
		if (!mpic_percpu_base_vaddr)
			printk(KERN_ERR "ehv_pic_init: of_iomap failed\n");
	...
	ehv_pic->hc_irq.set_affinity = ehv_pic_set_affinity;
